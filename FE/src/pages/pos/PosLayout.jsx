...
function PosSidebar() {
  ...
  return (
    <>
      <aside className="flex h-full w-[var(--pos-sidebar-width)] shrink-0 flex-col border-r border-[var(--admin-border)] bg-[var(--admin-surface)]">
        ...
        <div className="border-b border-[var(--admin-border)] px-4 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#0058be]/10 text-xs font-bold text-[var(--admin-brand)]">
              {(user?.name ?? 'Cashier')
                .split(' ')
                .slice(0, 2)
                .map((part) => part[0])
          ...
          {NAV.map((item) => (
            <NavLink
              ...
              className={({ isActive }) => {
                const active =
                  isActive ||
                  (item.to === '/pos' && location.pathname.startsWith('/pos/payment/'));
                return [
        ...
        <div className="border-t border-[var(--admin-border)] p-3">
          <button
            ...
            className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-[var(--admin-muted)] transition hover:bg-[#f0f4f8] hover:text-[var(--admin-text)]"
          >
    ...
    <PosCartProvider>
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="flex h-[var(--header-height)] shrink-0 items-center justify-between border-b border-[var(--admin-border)] bg-white px-5">
